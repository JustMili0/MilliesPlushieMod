...
public class MCTModelProvider extends FabricModelProvider {
    public MCTModelProvider(FabricPackOutput output) {
        super(output);
    }
    @Override
    ...
    private static BlockModelDefinitionGenerator createPlushBlockstates(Block plushBlock) {
        Identifier id = MCTPlushies.asResource("block/" + BuiltInRegistries.BLOCK.getKey(plushBlock).getPath());
